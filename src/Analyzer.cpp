...
// Preprocess1 : split the basicblock 
//			if it has more than on call instruction
	if(Preprocess1() != true){
		return false;
	}
  	...
  	for (auto iter1 = m->getFunctionList().begin(); iter1 != m->getFunctionList().end(); iter1++) {
		...
		for (auto iter2 = f.getBasicBlockList().begin(); iter2 != f.getBasicBlockList().end(); iter2++) {
			...
			for (auto iter3 = bb.begin(); iter3 != bb.end(); iter3++)
			{
				...
				unsigned int inst_energy = 0;

				if(inst.getOpcodeName() == std::string("alloca"))
				{
					std::cout << "[DEC]alloca instrution: " << ENERGY_INST_ALLOCA << std::endl;
					inst_energy = ENERGY_INST_ALLOCA;
				}
				else if(inst.getOpcodeName() == std::string("store"))
				{
					std::cout << "[DEC]store instrution: " << ENERGY_INST_STORE << std::endl;
					inst_energy = ENERGY_INST_STORE;
				}
				else if(inst.getOpcodeName() == std::string("load"))
				{
					std::cout << "[DEC]load instrution: " << ENERGY_INST_LOAD << std::endl;
					inst_energy = ENERGY_INST_LOAD;
				}
				else if(inst.getOpcodeName() == std::string("add"))
				{
					std::cout << "[DEC]add instrution: " << ENERGY_INST_ADD << std::endl;
					inst_energy = ENERGY_INST_ADD;
				}
				else if(inst.getOpcodeName() == std::string("mul"))
				{
					std::cout << "[DEC]mul instrution: " << ENERGY_INST_MUL << std::endl;
					inst_energy = ENERGY_INST_MUL;
				}
				else if(inst.getOpcodeName() == std::string("sdiv"))
				{
					std::cout << "[DEC]sdiv instrution: " << ENERGY_INST_SDIV << std::endl;
					inst_energy = ENERGY_INST_SDIV;
				}
				else if(inst.getOpcodeName() == std::string("ret"))
				{
					std::cout << "[DEC]ret instrution: " << ENERGY_INST_RET << std::endl;
					inst_energy = ENERGY_INST_RET;
				}
				else if(inst.getOpcodeName() == std::string("br"))
				{
					std::cout << "[DEC]br instrution: " << ENERGY_INST_BR << std::endl;
					inst_energy = ENERGY_INST_BR;
				}
				else if(inst.getOpcodeName() == std::string("icmp"))
				{
					std::cout << "[DEC]icmp instrution: " << ENERGY_INST_ICMP << std::endl;
					inst_energy = ENERGY_INST_ICMP;
				}
				else
				{
					std::cout << "[DEC]undefined instrution: " << inst.getOpcodeName() << std::endl;
				}
				
				wbb->addInstructionEnergy(inst_energy);
				std::cout << "[DEC]Total: " << wbb->eb << std::endl;
